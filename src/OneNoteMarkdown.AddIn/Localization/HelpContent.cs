    ...
    internal static class HelpContent
    {
        ...
        private static string GetAboutZh()
        {
            return "<h1>关于 OneNote Markdown</h1>"
                + "<p class='version'>版本：1.2.1-fix.1</p>"
                + "<h2>开发背景</h2>"
                ...
                + "<li>托管对照预览 —— 整页、选区或正文框可在右侧/下方独立更新</li>"
        ...
        private static string GetQuickStartZh()
        {
            return "<h1>快速入门</h1>"
                ...
                + "<h2>第二步：安装插件</h2>"
                + "<ol>"
                + "<li>32 位 OneNote → 运行 <code>OneNoteMarkdownSetup-1.2.1-fix.1-x86.exe</code></li>"
                + "<li>x64 OneNote → 运行 <code>OneNoteMarkdownSetup-1.2.1-fix.1-x64.exe</code></li>"
                + "<li>Windows 11 原生 Arm64 OneNote → 运行 <code>OneNoteMarkdownSetup-1.2.1-fix.1-arm64.exe</code>（要求 .NET Framework 4.8.1）</li>"
                ...
                + "<li>源码右侧或下方将创建托管预览；再次渲染会原位更新</li>"
                + "</ol>"
                + "<h3>方式二：当前正文框预览</h3>"
                + "<ol>"
                + "<li>把光标放入一个正文框并点击 \"渲染正文框\"</li>"
                + "<li>可在设置中选择右侧/下方、间距、宽度与可选自动刷新</li>"
                + "<li>预览标题可直接修改或删除，刷新不会强制恢复</li>"
                + "</ol>"
                + "<h2>卸载</h2>"
        ...
        private static string GetAboutEn()
        {
            return "<h1>About OneNote Markdown</h1>"
                + "<p class='version'>Version: 1.2.1-fix.1</p>"
                + "<h2>Background</h2>"
        ...
        private static string GetQuickStartEn()
        {
            return "<h1>Quick Start</h1>"
                ...
                + "<ol>"
                + "<li>32-bit OneNote → Run <code>OneNoteMarkdownSetup-1.2.1-fix.1-x86.exe</code></li>"
                + "<li>x64 OneNote → Run <code>OneNoteMarkdownSetup-1.2.1-fix.1-x64.exe</code></li>"
                + "<li>Native Arm64 OneNote on Windows 11 → Run <code>OneNoteMarkdownSetup-1.2.1-fix.1-arm64.exe</code> (.NET Framework 4.8.1 required)</li>"
        ...
        private static string GetIoEn()
        {
            return "<h1>Import/Export</h1>"
                ...
                + "<p>Import an external <code>.md</code> file as editable source and create a managed preview on its right.</p>"
                + "<h3>Steps</h3>"
                + "<ol>"
                ...
                + "<li>The plugin keeps editable Markdown source on the left and creates an updatable managed preview on the right</li>"
